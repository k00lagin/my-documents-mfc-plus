			...
			<tr>
				...
				{#if $settings["fillTemplates"] && importerOfficeNames?.length > 0}
					<th>
						...
						>
							<svg
								use:inlineSvg={iconStore["raphael_connect"]}
								class="frame-1"
							/>
							<svg
								use:inlineSvg={iconStore["raphael_disconnect"]}
								class="frame-2"
							/>
						</button>
					...
					<td>
						{#each office.type === "mfc" ? ["federal", "municipal", "regional", "otherServices"] : ["federal", "otherServices"] as template}
							<TemplateButton
								...
								fillData={importedData?.[office.pentaho_name]?.[
									template !== "otherServices" ? template : "federal"
								] || undefined}
								type={template}
